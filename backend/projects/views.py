...
from .serializers import ProjectListSerializer, ProjectDetailSerializer, ProjectStageSerializer
...
def _normalize_project_stage_order(project_id: int) -> List[Dict[str, Any]]:
    ...
    with transaction.atomic():
        rows = list(ProjectStage.objects.select_for_update().filter(project_id=project_id).order_by('order','id'))
        for idx, row in enumerate(rows, start=1):
            if row.order != idx:
                row.order = idx
                row.save(update_fields=['order'])
    # Возвращаем сериализованные данные (для удобного ответа)
    data = ProjectStageSerializer(ProjectStage.objects.filter(project_id=project_id).order_by('order','id'), many=True).data
    return data
...
class ProjectViewSet(mixins.ListModelMixin,
    ...
    def stages(self, request, pk=None):
        if request.method.lower() == 'get':
            def _list():
                qs = ProjectStage.objects.filter(project_id=pk).order_by('order','id')
        ...
        def _create():
            ...
            return Response({'created': ProjectStageSerializer(obj).data, 'count': len(stages), 'stages': stages},
    ...
    def stages_reorder(self, request, pk=None):
        ...
        if not isinstance(ids, list) or not all(isinstance(x, int) for x in ids):
            return Response({'detail': 'Ожидается JSON: {"ids": [int, ...]}'}, status=status.HTTP_400_BAD_REQUEST)
        with transaction.atomic():
            # Установим временно большие номера, чтобы избежать конфликтов уникальности, если появится
            base = 1000000
            for i, sid in enumerate(ids, start=1):
                ProjectStage.objects.filter(project_id=pk, id=sid).update(order=base + i)
        stages = _normalize_project_stage_order(int(pk))
        return Response({'count': len(stages), 'stages': stages}, status=status.HTTP_200_OK)


class ProjectStageViewSet(mixins.ListModelMixin,
                          mixins.RetrieveModelMixin,
                          mixins.CreateModelMixin,
    ...
    def create(self, request, *args, **kwargs):
        ...
        def _create():
            ...
            _normalize_project_stage_order(int(ser.validated_data.get('project').id if hasattr(ser.validated_data.get('project'),'id') else ser.validated_data.get('project')))
    ...
    def update(self, request, *args, **kwargs):
        ...
        def _update():
            ...
            with transaction.atomic():
                obj = ser.save()
            # Нормализуем после изменения order
            _normalize_project_stage_order(int(obj.project_id))
            # Вернём актуальный список по проекту для удобства
            data = ProjectStageSerializer(ProjectStage.objects.filter(project_id=obj.project_id).order_by('order','id'), many=True).data
            return Response({'updated': ProjectStageSerializer(obj).data, 'stages': data}, status=status.HTTP_200_OK)
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        def _destroy():
            ...
            return Response({'deleted': True, 'stages': stages}, status=status.HTTP_200_OK)
