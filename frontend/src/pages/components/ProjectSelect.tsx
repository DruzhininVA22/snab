/**
 * Выбор проекта (Project).
 * 
 * Autocomplete с поиском по проектам.
 * Загружает проекты с API, отображает код и название.
 */
...
export default function ProjectSelect({
    ...
    React.useEffect(() => {
        if (open) {
            const h = window.setTimeout(() => load(input.trim()), 200);
    ...
    React.useEffect(() => {
        ...
        (async () => {
            try {
                const data = await http.get(fixPath(`/api/projects/projects/${value}/`));
                const pr = {
                    id: data.id,
                    name: data.name ?? String(data.id),
                    code: data.code,
                };
                if (alive) setOptions((prev) => [pr, ...prev]);
            } catch {
                // ignore
            }
        })();
