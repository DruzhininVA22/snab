/**
 * Выбор этапа проекта (ProjectStage).
 * 
 * Загружает этапы для указанного projectId.
 * Fallback к разным URL если основной не работает.
 */
...
export default function ProjectStageSelect({
    ...
    const load = React.useCallback(
        async (q: string, pid: number | null) => {
            if (!pid) {
                setOptions([]);
                return;
            }
            setLoading(true);
            try {
                setOptions(await tryEndpoints(pid, q));
            } finally {
                setLoading(false);
            }
        },
    ...
    React.useEffect(() => {
        if (open) {
            const h = window.setTimeout(() => load(input.trim(), projectId), 200);
            return () => window.clearTimeout(h);
        }
    }, [open, input, projectId, load]);
        ...
        <Autocomplete
            ...
            renderInput={(params) => (
                <TextField
                    ...
                    size="small"
                    label={label || 'Этап'}
                    placeholder={projectId ? 'Начните вводить...' : 'Сначала выберите проект'}
                    InputProps={{
                        ...params.InputProps,
